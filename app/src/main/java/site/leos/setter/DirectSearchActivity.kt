...
class DirectSearchActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        query_type_rg.apply {
            check(if (PreferenceManager.getDefaultSharedPreferences(context).getBoolean(getString(R.string.meta_search_key), true)) R.id.meta_query else R.id.browser_query)
            setOnCheckedChangeListener { _, i -> if (edit_query.text.isNotEmpty()) searchIt(edit_query.text.toString(), i==R.id.meta_query) }
        }
    ...
    private fun searchIt(query: String, meta: Boolean) : Boolean {
        if (query.isNotBlank()) {
            ...
            if (meta) {
                ...
                else startActivity(Intent(this, WebSearchActivity::class.java).putExtra(WebSearchActivity.QUERY, query).putExtra(WebSearchActivity.META, true))
            } else {
